var container=$(".container");
    ...
    var canvas_href=document.getElementById("canvas_href");
    ...
    cut.click(function(){
        type=$(this).attr("data");
        typechoose.removeClass("typeactive");
        $(this).css({color:"#28AAAA",backgroundColor:"#fff"}).toggleClass("shezhistyle");
        iscut=true;
    })
    copy.click(function(){
        type="cut";
    ...
    //     }else{
    //         $(ele).click(function(){
    //             $(this).css({color:"#28AAAA",backgroundColor:"#fff"}).animate({opacity:0.99},200,function(){
    //                 $(this).css({color:"#fff",backgroundColor:"#28AAAA",opacity:1});
    //             });
    //         })
    //     }
    // })
    ...
    save.click(function(){
       ...
       var reg=canvas.toDataURL("image/png");//直接自动保存下载
       canvas_href.href = reg;
       canvas_href.click();
    //    console.log(reg);
    //    console.log(reg);
        // location.href=reg;
    ...
    ding.onclick=function(e){
        canvas.width=width;
        canvas.height=height;
        canvas.style.left=(screenWidth-150-canvas.width)/2+"px";
        canvas.style.top=(screenHeight-100-canvas.height)/2+"px";
        arr=[];
        obj.clearRect(0,0,width,height);
        e.stopPropagation()
        xinjian.fadeOut();
